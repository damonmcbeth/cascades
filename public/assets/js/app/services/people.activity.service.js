 (function() {
	...
	app.factory('peopleActivityService', ['$q', '$filter', 'activityService', 'peopleService', 'globalSettings', 
		function($q, $filter, activityService, peopleService, globalSettings) {
		
		function PeopleActivityService() {
			var self = this;
			...
			self.save = function (edited, orig) {
				...
				peopleService.clonePerson(orig).then(
					function(origClone) {
						peopleService.savePerson(edited, orig).then(
							function(result) {
								self.captureUpdateActivity(edited, origClone);
								globalSettings.showSuccessToast(globalSettings.currWorkspace.Terminology.clientAlias + ' saved.');
		...
		return new PeopleActivityService();
	}]);
